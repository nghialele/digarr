...
import { autoApprove, type AutoApproveDeps } from '@/core/pipeline/auto-approve'
  ...
  return {
    ...
    getEnabledTargets: vi.fn().mockResolvedValue([{
      ...
      addArtist: vi.fn().mockResolvedValue({
        success: true, targetType: 'lidarr', targetId: 1, externalId: 99,
      }),
...
describe('autoApprove()', () => {
  it('approves recommendations above threshold via targets', async () => {
    const deps = makeDeps()
    const result = await autoApprove(1, {
      threshold: 0.7,
      monitorOption: 'all',
      qualityProfileId: 1,
      metadataProfileId: 1,
  ...
  it('skips non-pending recommendations', async () => {
    ...
    const result = await autoApprove(1, {
      threshold: 0.5, monitorOption: 'all',
  ...
  it('handles no targets gracefully', async () => {
    ...
    expect(deps.updateRecommendationStatus).toHaveBeenCalledWith(
      1, 'approved', expect.objectContaining({ targetActions: {} }),
    )
  })

  it('continues on individual target failure', async () => {
    const failTarget = {
      id: 'lidarr-1', name: 'Lidarr', type: 'lidarr' as const, capabilities: ['addArtist' as const],
      addArtist: vi.fn()
        .mockResolvedValueOnce({ success: true, targetType: 'lidarr', targetId: 1, externalId: 10 })
        .mockResolvedValueOnce({ success: false, targetType: 'lidarr', targetId: 1, error: 'Already exists' }),
      testConnection: vi.fn(),
    }
    const deps = makeDeps({
